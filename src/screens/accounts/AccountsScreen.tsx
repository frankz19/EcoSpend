      ...
      <ScrollView style={{ padding: 20 }}>
        {accounts.map(acc => (
          <View key={acc.id} style={styles.card}>
            <Text>{acc.name}</Text>
            <Text style={{ fontWeight: 'bold' }}>${acc.current_balance.toFixed(2)}</Text>
          </View>
        ))}
...
const styles = StyleSheet.create({
  ...
  card: { padding: 20, backgroundColor: '#F5F5F5', borderRadius: 10, marginBottom: 10, flexDirection: 'row', justifyContent: 'space-between' },
